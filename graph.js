...
function buildOptimizedLoads(orders) {
  ...
  while (available.length) {
    ...
    (function doSomething(orderedOptions) {
      ...
      //   if (26 - (newLoad.pallets + orderedOptions[i].pallets) >= 0) {
      //     newLoad.route.push(orderedOptions[i]);
      //     orderedOptions.shift();
      //     previousCity = option.dropCity;
      //     available = available.filter(
      //       (element) => element.order !== option.order
      //     );
      //   } else {
      //     orderedOptions.filter((element) => element.order !== option.order);
      //   }
      // }
    })(orderedOptions);
